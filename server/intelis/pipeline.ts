...
async function sendResearchEmail(input: { email: string; subject: string; markdown: string }) {
  ...
  return response.ok ? { sent: true, detail: "Delivered by email provider." } : { sent: false, detail: `Email provider rejected delivery (${response.status}).` };
}
...
export function renderResearchEmailHtml(markdown: string) {
  const escaped = markdown
    ...
    .replace(/"/g, "&quot;")
    .replace(/'/g, "&#39;");
  return `<article style="font-family:ui-sans-serif,system-ui;line-height:1.6;max-width:680px">${escaped.replace(/\n/g, "<br/>")}</article>`;
}
...
export async function runResearchPipeline(task: ResearchTask, trigger: "scheduled" | "manual" | "retry") {
  ...
  try {
    ...
    await updatePipelineRun(runId, { status: candidates.length && !analyzed.length ? "partial" : "completed", currentStage: null, stageStates: states, sourceCount: candidates.length, findingCount: savedFindings.length, completedAt: new Date() });
